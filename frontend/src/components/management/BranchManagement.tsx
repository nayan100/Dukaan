...
const MOCK_BRANCHES: Branch[] = [
  { id: 'BR-001', name: 'Thamel Flagship', location: 'Kathmandu', pos_quota: 5, pos_used: 3, manager: 'Ram Prasad' },
  { id: 'BR-002', name: 'Pokhara Lakeside', location: 'Pokhara', pos_quota: 3, pos_used: 2, manager: 'Sita Devi' },
];
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {branches.map(branch => (
              <div key={branch.id} className="bg-pos-surface border border-pos-border rounded-3xl p-6 shadow-2xl hover:border-pos-primary/30 transition-all group relative overflow-hidden">
                <div className="flex justify-between items-start mb-6">
                  <div className="w-12 h-12 rounded-2xl bg-pos-black flex items-center justify-center text-pos-primary shadow-inner">
                    <Building2 size={24} />
                  </div>
                ...
                {/* Background Decor */}
                <div className="absolute -bottom-4 -right-4 text-pos-white/5 font-black text-6xl italic group-hover:text-pos-primary/5 transition-colors">
                  {branch.id.split('-')[1]}
                </div>
              </div>
            ))}
           ...
           <div className="max-w-md w-full bg-pos-surface border border-pos-border rounded-3xl p-8 shadow-2xl">
              ...
              <form className="space-y-6" onSubmit={(e) => { e.preventDefault(); setIsAdding(false); }}>
                 <div className="space-y-2">
                    ...
                    <input type="text" className="w-full bg-pos-black border border-pos-border rounded-xl p-4 text-pos-white font-bold" placeholder="e.g. Biratnagar Express" />
                 </div>
                 <div className="space-y-2">
                    <label className="text-[10px] font-black uppercase text-pos-muted tracking-widest pl-1">POS Allocation</label>
                    <input type="number" min="1" max={remainingPOS} className="w-full bg-pos-black border border-pos-border rounded-xl p-4 text-pos-white font-bold" placeholder="Seats to allocate" />
